...
| --- | --- | --- |
| `sonarcloud-token` | Secret text | `Jenkinsfile.ci` SonarCloud analysis and effective quality policy check |
...
For the Free Tier POC SonarCloud gate, `Jenkinsfile.ci` does not require the
token to administer or attach a server-side quality gate. Jenkins runs the
analysis, waits for the Sonar Compute Engine task, reads the returned quality
gate conditions, and applies the effective policy from `config/pipeline-policy.yml`,
including the 50 percent new-code coverage threshold.
...
Enable automatic SBOM generation once in Harbor under
`Projects > vault-bank > Configuration > SBOM generation`. In the Free Tier
pipeline Jenkins keeps `HARBOR_AUTO_SBOM_REQUIRED=0`, because Harbor API SBOM
polling needs artifact read permissions that are often not available to a
push-only robot account. The supply-chain gate is still enforced by the Syft
SBOM validation stage and the Cosign SPDX attestation stage. Set
`HARBOR_AUTO_SBOM_REQUIRED=1` only after the Harbor credential can read artifact
details through the Harbor API.
