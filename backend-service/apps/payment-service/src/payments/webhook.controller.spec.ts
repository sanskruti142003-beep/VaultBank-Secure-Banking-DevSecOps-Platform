...
describe('WebhookController Stripe webhook', () => {
  ...
  const applyGatewayStatus = jest.fn<Promise<void>, [string, PaymentStatus]>();

  const constructEvent = jest.fn<Stripe.Event, [Buffer, string, string]>();
  ...
  beforeEach(() => {
    process.env.STRIPE_SECRET_KEY = 'unit-test-stripe-key';
    ...
    controller = new WebhookController(payments, http);
  ...
  it.each([
    ['payment_intent.succeeded', 'pi_success', PaymentStatus.SUCCESS],
    ['payment_intent.payment_failed', 'pi_failed', PaymentStatus.FAILED],
    ['charge.refunded', 'ch_refunded', PaymentStatus.REFUNDED],
  ] as const)(
    'applies the correct status for %s',
    async (type, gatewayReference, expectedStatus) => {
      constructEvent.mockReturnValue(createStripeEvent(type, gatewayReference));

      await expect(
        controller.stripeWebhook(createRequest(), 'unit-test-signature'),
      ).resolves.toEqual({
        received: true,
      });
  ...
  it('acknowledges an unsupported Stripe event without changing payment status', async () => {
    constructEvent.mockReturnValue(
      createStripeEvent('customer.created', 'cus_test'),
    );
  ...
  it('rejects a request without a Stripe signature', async () => {
    await expect(
      controller.stripeWebhook(createRequest(), undefined),
  ...
  it('rejects a request without a raw request body', async () => {
    const requestWithoutRawBody = {} as RawBodyRequest<Request>;

    await expect(
      controller.stripeWebhook(requestWithoutRawBody, 'unit-test-signature'),
    ).rejects.toBeInstanceOf(BadRequestException);
